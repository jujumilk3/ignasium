...
async def post(
    ...
    content: str = Body(
        "content", description="content", example="content", embed=True
    ),
):
    return {
...
async def delete(
    title: str = Query("title", description="title", example="title", embed=True),
    content: str = Query(
        "content", description="content", example="content", embed=True
    ),
):
...
async def get_page_num_as_requested(
    ...
    limit: int = Query(20, description="limit", example=20),
):
    return {"hello": "world"}
